...
with open(dependencies_target_file, "w") as target:
    ...
    target.write("nsudo=$1\n")
    target.write("binstaller=$2\n")
    # target.write("echo nsudo=$nsudo binstaller=$binstaller\n")
    ...
    for key, items in dependencies.items():
        ...
        for item in items:
            ...
            for install_command in item["install_command"]:
                ...
                target.write(f"      fi\n")
                target.write(
                    f"      {install_command} $installer_options || (test $? -ne 0 && $nsudo {install_command} $installer_options) &\n"
                )
                target.write(
